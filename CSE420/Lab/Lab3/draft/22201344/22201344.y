...
variable_decl : type_specifier declaration_list SEMICOLON
		 {
			...
			for(unsigned int i = 0; i < decl_id_list.size(); i++)
			{
				...
				if(!table->insert(var_sym))
				{
					...
					if(existing != NULL)
					{
						existing->set_data_type($1->get_name());
						if(decl_id_list[i].second > 0)
						{
...
variable : ID 	
      {
		...
		{
			$$ = new symbol_info($1->get_name(),"varbl",found_sym->get_data_type());
		}
	 ...
	 | ID LTHIRD expression RTHIRD 
	 {
		...
		else if(found_sym2->get_symbol_type() != "Array")
		{
			...
			$$ = new symbol_info($1->get_name()+"["+$3->get_name()+"]","varbl",found_sym2->get_data_type());
		}
		else
		{
			if($3->get_data_type() != "int" || found_sym2->get_data_type() != "int")
			{
...
arguments : arguments COMMA logic_expression
		  {
				...
				call_arg_stack.back().push_back($3->get_data_type());
		  }
	      | logic_expression
	      {
				...
				call_arg_stack.back().push_back($1->get_data_type());
		  }
	      ;
