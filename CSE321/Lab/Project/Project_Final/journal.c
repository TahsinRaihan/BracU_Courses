...
void cmd_create(const char *filename) {
    ...
    if (sb->inode_bitmap != sb->journal_block + JOURNAL_BLOCKS) die("Invalid inode bitmap layout");
    if (sb->data_bitmap != sb->inode_bitmap + 1) die("Invalid data bitmap layout");
    if (sb->inode_start != sb->data_bitmap + 1) die("Invalid inode table layout");
    if (sb->data_start != sb->inode_start + INODE_BLOCKS_CNT) die("Invalid data start layout");
    ...
    if (!is_valid_data_block(sb, root_data_blk_idx)) die("Root directory block invalid after replay");
