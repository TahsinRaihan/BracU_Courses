<script context="module">
  ...
  export async function load({ params }) {
    const id = params.id;
    const { data: item, error: itErr } = await supabaseClient
      .from('menu_items')
      .select('*')
      .eq('id', id)
      .single();
    if (itErr || !item) throw error(404, 'Dish not found');
...
<script>
  ...
  async function placeOrder() {
    ...
    // Client-only counter for display
      ...
      {#if reviews.length}
        {#each reviews as r}
          <div class="p-4 border rounded">
            <div class="font-semibold">{r.rating}★</div>
            <p>{r.comment}</p>
          </div>
        {/each}
      {:else}
