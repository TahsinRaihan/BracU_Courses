	...
	try {
      ...
      `
		...
		const availableCopies =
			book.book_copies?.filter((copy) => copy.status === 'available' && copy.is_available === true)
				?.length || 0;
...
export const DELETE: RequestHandler = async (event) => {
	try {
		...
		console.log('🗑️ DELETE: Starting deletion for book ID:', bookId);
		...
		if (fetchError) {
			console.error('❌ DELETE: Error fetching book:', fetchError);
			return json({ success: false, error: 'Book not found' }, { status: 404 });
		}
		...
		console.log('✅ DELETE: Found book:', book.title);

		// Delete book copies first (due to foreign key constraints)
		console.log('🗑️ DELETE: Deleting book copies...');
		const { error: copiesError } = await supabase
			.from('book_copies')
			.delete()
			.eq('book_id', bookId);
		...
		if (copiesError) {
			console.error('❌ DELETE: Error deleting book copies:', copiesError);
			return json({ success: false, error: 'Failed to delete book copies' }, { status: 500 });
		}

		console.log('✅ DELETE: Book copies deleted successfully');

		// Delete the book
		console.log('🗑️ DELETE: Deleting book...');
		const { error: bookError } = await supabase.from('books').delete().eq('id', bookId);

		if (bookError) {
			console.error('❌ DELETE: Error deleting book:', bookError);
			return json({ success: false, error: 'Failed to delete book' }, { status: 500 });
		}

		console.log('✅ DELETE: Book deleted successfully:', book.title);
		return json({
			success: true,
