<script lang="ts">
	...
	$: suggestions = [
		...
		...data.items.map((i: MenuItem) => i.name)
	];
...
<div class="h-full overflow-y-auto">
	...
	>
		<div class="absolute inset-0 flex items-center justify-center bg-black/60">
			<h1 class="text-5xl font-black text-white italic">"Savor the Flavors of Home"</h1>
		</div>
	</div>

	<!-- Search Bar -->
	<SearchBar bind:search {suggestions} on:input={debounceSearch} />

	<!-- Category Pills -->
	<div class="sticky top-20 z-10 overflow-x-auto bg-white/90 py-2 backdrop-blur">
		<div class="flex gap-2 px-4">
			<button
				...
				class="rounded-full border bg-gray-200 px-4 py-2">All</button
			>
			{#each categories as c, idx (c.id)}
				{#if grouped[c.id]?.length}
					<button
						...
						class={`rounded-full border px-4 py-2 ${pillColors[idx % pillColors.length]}`}
					>
						{c.name}
					</button>
				{/if}
	...
	{#each categories as c, idx (c.id)}
		{#if filteredGrouped[c.id]?.length}
			<section
				id={'cat-' + c.id}
				class={`px-6 py-8 ${sectionBg[idx % sectionBg.length]} mx-4 mb-8 rounded-lg shadow-lg`}
			>
				<h2 class="mb-4 text-3xl font-bold">{c.name}</h2>
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
					{#each filteredGrouped[c.id] as item (item.id)}
						<a
							href={`/cafeteria/item/${item.id}`}
							class="relative block overflow-hidden rounded-lg border bg-white shadow-md transition-all duration-300 hover:shadow-lg"
