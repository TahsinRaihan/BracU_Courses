...
<script lang="ts">
  ...
  export let data;
    let { categories, items, grouped }: { categories: any[], items: any[], grouped: any } = data; // eslint-disable-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  ...
  $: suggestions = [...data.categories.map(c => c.name), ...data.items.map(i => i.name)];
...
<div class="h-full overflow-y-auto">
  ...
  <!-- Search Bar -->
  <SearchBar bind:search={search} suggestions={suggestions} on:input={() => { 
    ...
    {#if grouped[c.id]?.length}
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {#each grouped[c.id] as item (item.id)}
            <a
              href={`/item/${item.id}`}
