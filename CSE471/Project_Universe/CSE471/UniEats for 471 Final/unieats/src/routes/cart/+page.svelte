<script lang="ts">
  ...
  export let data;
  let localCart: any[] = []; // eslint-disable-line @typescript-eslint/no-explicit-any
  let localScheduled: any[] = []; // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  async function cancelOrder(orderId: string, type: 'direct' | 'scheduled') {
    ...
    // Perform the actual cancellation in the backend
