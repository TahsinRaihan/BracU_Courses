...
for gen in range(1, generations + 1): # Evaluate fitness
    ...
    while len(next_gen) < pop_size: #geneate child until populaton
        ...
        cut1, cut2 = sorted(random.sample(range(1, len(flat1)), 2)) # select two cut points 
        child1_flat = flat1[:cut1] + flat2[cut1:cut2] + flat1[cut2:] #c1= parent 1 + parent2+ parent1
        child2_flat = flat2[:cut1] + flat1[cut1:cut2] + flat2[cut2:] #c2= parent 2 + parent1+ parent2


        child1 = [(child1_flat[i], child1_flat[i+1]) for i in range(0, len(child1_flat), 2)]  # Re bulding the chuld into 6 point pair
        child2 = [(child2_flat[i], child2_flat[i+1]) for i in range(0, len(child2_flat), 2)]

        
        if random.random() < mutation_rate: # mutation on child1
            bi = random.randrange(len(child1))
            weidth, height = blocks[bi][1], blocks[bi][2]
            child1[bi] = (random.randint(0, grid_size - weidth),random.randint(0, grid_size - height))
        next_gen.append(child1)

        
        if len(next_gen) < pop_size and random.random() < mutation_rate: # mutation on child2
            bi = random.randrange(len(child2))
            weidth, height = blocks[bi][1], blocks[bi][2]
            child2[bi] = (random.randint(0, grid_size - weidth),random.randint(0, grid_size - height))
...
(best_fit, best_ov, best_wire, best_area), best_chrom = best_solution
