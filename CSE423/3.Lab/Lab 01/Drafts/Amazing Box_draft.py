...
# Initialize GLUT
glutInit()  # Add this line

# Window dimensions (larger size)
W_Width, W_Height = 1200, 800  # Larger window size
...
# Main function
glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB)  # Set the display mode (double buffering and RGB)
glutInitWindowSize(W_Width, W_Height)  # Set the window size

# Set the window position to top-left corner (0, 0)
glutInitWindowPosition(0, 0)  # Change this line

glutCreateWindow(b"Amazing Box")  # Create the window with the title "Amazing Box"

init()  # Initialize OpenGL settings

glutDisplayFunc(display)  # Set the display callback function
glutIdleFunc(animate)  # Set the idle callback function (for animation)
glutKeyboardFunc(keyboardListener)  # Set the keyboard callback function
glutSpecialFunc(specialKeyListener)  # Set the special key callback function (for arrow keys)
glutMouseFunc(mouseListener)  # Set the mouse callback function
