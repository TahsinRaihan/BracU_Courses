...
def draw_pause_button(x, y):
    color = (1.0, 0.75, 0.0)  # Amber color
    
    if is_paused:
        # Draw play icon (right-pointing triangle)
        # Left vertical line - using multiple points
        for dy in range(-15, 16, 2):  # From y-15 to y+15 in steps of 2
            draw_points(x - 10, y + dy, color)
        
        # Diagonal lines - need to properly plot them
        # Top diagonal (from top-left to right-center)
        mpl(x - 10, y + 15, x + 10, y, color)
        # Bottom diagonal (from bottom-left to right-center)
        mpl(x - 10, y - 15, x + 10, y, color)
    else:
        # Draw pause icon (two proper vertical lines)
        # Left line - using multiple points
        for dy in range(-15, 16, 2):  # From y-15 to y+15 in steps of 2
            draw_points(x - 10, y + dy, color)
        
        # Right line - using multiple points
        for dy in range(-15, 16, 2):  # From y-15 to y+15 in steps of 2
            draw_points(x + 10, y + dy, color)
def draw_cross(x, y):
    color = (1.0, 0.0, 0.0)
...
def mouse_click(button, state, x, y):
    ...
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
        y = screen_height - y
        if (screen_width - 60 <= x <= screen_width - 10) and (screen_height - 60 <= y <= screen_height - 10):
            print(f"Goodbye. Final Score: {score}")
            glutLeaveMainLoop()
        elif (screen_width // 2 - 30 <= x <= screen_width // 2 + 30) and (screen_height - 60 <= y <= screen_height - 10):
            if not game_over:
                is_paused = not is_paused
        elif (10 <= x <= 70) and (screen_height - 60 <= y <= screen_height - 10):
            print("Starting Over")
            game_over = False
            score = 0
            diamond_speed = 150
