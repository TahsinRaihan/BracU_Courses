...
def draw_diamond(x, y):
    if not game_over:
        mpl(x, y + 17, x - 12, y, diamond_color)
        mpl(x, y + 17, x + 12, y, diamond_color)
        mpl(x, y - 17, x - 12, y, diamond_color)
        mpl(x, y - 17, x + 12, y, diamond_color)
...
def draw_catcher(x, y):
    ...
    mpl(x - 70, y, x + 70, y, color)        # top
    mpl(x - 70, y, x - 60, y - 20, color)   # left
    mpl(x + 70, y, x + 60, y - 20, color)   # right
    mpl(x - 60, y - 20, x + 60, y - 20, color)  # bottom
...
def draw_pause_button(x, y):
    color = (1.0, 0.75, 0.0)
    if is_paused:
        for dy in range(-15, 16, 2):  # pause triangle
...
def update(value):
    ...
    if not is_paused and not game_over:
        ...
        diamond_y -= int(5 + (140 - diamond_speed) * 0.1)
        if diamond_y < 0:
            game_over = True
...
def mouse_click(button, state, x, y):
    ...
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
        y = screen_height - y
        if (screen_width - 60 <= x <= screen_width - 10) and (screen_height - 60 <= y <= screen_height - 10): #Cross button position
            print(f"Goodbye. Final Score: {score}")
            glutLeaveMainLoop()
        elif (screen_width // 2 - 30 <= x <= screen_width // 2 + 30) and (screen_height - 60 <= y <= screen_height - 10): #pause button position
            if not game_over:
                is_paused = not is_paused
        elif (10 <= x <= 70) and (screen_height - 60 <= y <= screen_height - 10): #Return button position
            print("Starting Over")
            game_over = False
            score = 0
            diamond_speed = 150
...
glutTimerFunc(diamond_speed, update, 0)
