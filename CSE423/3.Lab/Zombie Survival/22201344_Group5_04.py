...
class Particle: # handles blood splatters
    def __init__(self, x, y, z):
        self.x = x + random.uniform(-5, 5)
        self.y = y + random.uniform(0, 5)
        self.z = z + random.uniform(-5, 5)
        self.size = random.uniform(1, 3)   # random size spaltter from center
...
class Bullet:  # draws and calculats distance of bullet firing
...
class Enemy:
    ...
    def update(self): #calculates x,y,z distance of enemy and player
        ...
        if dist > 0:
            ...
            if inside_boundary(nx, nz) and not is_inside_obstacle(nx, nz): #makes sure doesnot go outside boundary
                self.x, self.z = nx, nz
    ...
    def check_collision_with_player(self):
        dx = player_pos[0] - self.x
        dz = player_pos[2] - self.z
        return math.hypot(dx, dz) < 20 #if within 20 units the collicion
...
def draw_barrel_fp(): #Gun barrel draw for fov
...
def draw_gun():
    ...
    if follow_camera:
        ...
        glColor3f(0.2, 0.2, 0.2)
        quad = gluNewQuadric()
        gluCylinder(quad,0.1,0.1,1.5,12, 3)
        glPopMatrix()

    else:
        glColor3f(0.7, 0.7, 0.7)
        glTranslatef(0, 45, 12)
        quad = gluNewQuadric()
        gluCylinder(quad, 3.5, 2, 25, 12, 2)
...
def draw_status(): #All visual things meaning scoreboard and notifications
    ...
    if shield_active and shield_timer > 0:
        draw_text(x0, stats_y + 20,f"Shield On ({shield_timer//60}s)")
...
def setupCamera():
    ...
    if follow_camera:
        eye_x = player_pos[0]
        eye_y = player_pos[1]  + 35   #position in body height
        eye_z = player_pos[2]
        ...
        gluLookAt(cam_x, cam_y, cam_z,player_pos[0], player_pos[1] + player_height, player_pos[2],0, 1, 0)
...
def fire_bullet(): # in cheat mode closest enemy calculate and fire
    global bullets
...
class BloodParticle: # red cubes if zombie dies
    def __init__(self, x, y, z):
...
def update_game():   # Handels all kind of update power ups, orb taken, bullet misss, score etc
...
def reset_game(): # when restart all variable becomes same again
...
def idle(): #Closest enemy calculate then angle rotation
...
def init(): #lighting setup and ensure enemy spawn doenot spawn at obstackle
...
def showScreen(): #renders all visible elements
