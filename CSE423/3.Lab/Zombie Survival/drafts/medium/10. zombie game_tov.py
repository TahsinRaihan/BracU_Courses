...
class Pickup:
    def __init__(self, x, z, color):
        ...
        self.active = True
        # both health and weapon orbs now live “forever”
        self.timer = 999999

    def update(self):
        # only ever expires if you manually deactivate it
...
def draw_status():
    ...
    # misses left
    draw_text(10, 100,
        f"Misses left: {MAX_MISSED_BULLETS - bullets_missed}/{MAX_MISSED_BULLETS}"
    )

    # — Big notification (bitmap Times Roman 24) —
    if notification_timer > 0:
        glColor3f(*notification_color)
        text = notification_text
        text_width = len(text) * 14   # slightly wider per char
        x = (w - text_width) / 2
        y = h * 0.3
        glRasterPos2f(x, y)
        for ch in text:
...
def update_game():
    ...
    for pw in power_ups[:]:
        ...
        if not pw.active:
            ...
            power_ups.remove(pw)
            show_notification("Weapon orb taken!",pw.color)
...
def show_notification(text, color=(1,0,0)):
    ...
    notification_timer = 600    # still plenty of frames
