...
class Enemy:
    """
    Represents a single zombie. 
    - type: 'normal', 'runner', or 'tank'
    - is_dead: whether it’s been killed
    - fall_y: corpse drop offset
    - pulses in size
    - won’t move through obstacles
    """
    ...
    def respawn(self, value=None):
        """
        Place at a random edge; reset corpse offset; 
        scale speed +10% per wave (cap wave 10); reset state.
        """
        global current_wave
        ...
        self.speed = random.uniform(0.02, 0.2) * (1 + (lvl - 1) * 0.1)
    ...
    def update(self):
        """
        Move toward the player unless blocked by an obstacle.
        Pulses the scale.
        """
        global game_paused

        if game_paused or self.is_dead:
            return
        ...
        new_z = self.z + (dz / dist) * self.speed

        if not is_inside_obstacle(new_x, new_z):
            self.x = new_x
            self.z = new_z
    ...
    def draw(self):
        """
        Draws the body, head, eyes (if alive), and arms.
        Uses self.fall_y so corpses can drop.
        """
        glPushMatrix()
        ...
        if not self.is_dead:
            ...
            for ex in (-2.5, 2.5):
                glPushMatrix()
                glTranslatef(ex, height + 11, eye_off)
        ...
        arm_l  = 16 if self.type != 'tank' else 20
        arm_col = (0.4, 0.3, 0.2) if not self.is_dead else (0.2, 0.1, 0.1)
        glColor3f(*arm_col)
        for ax in (-13, 13):
            glPushMatrix()
            glTranslatef(ax, arm_h, 0)
            glRotatef(10 if ax < 0 else -10, 1, 0, 0)
            glScalef(arm_th, arm_l, arm_th)
            glutSolidCube(1)
            glPopMatrix()
    ...
    def check_collision_with_player(self):
        """
        Returns True if close enough to damage player.
        """
        if self.is_dead:
            return False
        dx = player_pos[0] - self.x
    ...
    def check_collision_with_bullet(self, bullet):
        """
        Returns True if bullet is in range to kill this enemy.
        """
        if self.is_dead:
            return False
        dx = bullet.x - self.x
...
def draw_gun():
    ...
    if follow_camera:
        # First-person view with partial arms and gun
        glPushMatrix()  # For player position/rotation
        ...
        # Left arm (supporting)
        glPushMatrix()  # Left arm matrix
        glTranslatef(-6, 18, -8)
        glRotatef(30, 0, 1, 0)
        gluCylinder(quad, 2.5, 2.0, 20, 12, 2)
        glPopMatrix() 
        
        # Gun (positioned between arms)
        glPushMatrix()  # Gun matrix
        ...
        # Barrel
        glPushMatrix()  # Barrel matrix
        glTranslatef(0, 0, 5)
        gluCylinder(quad, 1.5, 1.3, 25, 16, 2)
        glPopMatrix()  # Barrel matrix pop
        
        # Handle
        glPushMatrix()  # Handle matrix
        glTranslatef(0, 1.5, 10)
        glRotatef(90, 1, 0, 0)
        gluCylinder(quad, 1.8, 1.8, 8, 12, 2)
        glPopMatrix()  # Handle matrix pop
        
        glPopMatrix()  # Gun matrix pop
        
        # Partial head/helmet (top portion visible)
        glPushMatrix()  # Head matrix
        glTranslatef(0, 25, -10)
        glColor3f(0, 0, 0)  # Black helmet
        glutSolidSphere(8, 16, 16)
        glPopMatrix()  # Head matrix pop
        
        glPopMatrix()  # Player position/rotation pop
    else:
        # Third-person view
...
def draw_status():
    """
    Top‐of‐screen UI:
      • Health bar (green/yellow/red), with “Health: X/Y” drawn to its right
      • Level bar (blue), with “Level: N/10” drawn to its right
      • Score, Weapon Lv, and Misses‐left on one horizontal row
      • Big centered notifications in Times Roman 24
    ...
    hc = (0,1,0) if frac_h>0.66 else ((1,1,0) if frac_h>0.33 else (1,0,0))
    glColor3f(1,1,1)
    ...
    # Health label to the right
    label_x = x0 + bar_w + 10
    draw_text(label_x, y0 + bar_h - 5, f"Health: {player_health}/{max_health}")
    ...
    glVertex2f(x0 + bar_w, level_y0)
    glVertex2f(x0 + bar_w, level_y0 + bar_h)
    ...
    # Level label to the right
    draw_text(label_x, level_y0 + bar_h - 5, f"Level: {current_wave}/10")

    # — Score / Weapon Lv / Misses‐Left (horizontal row) —
...
def update_game():
    ...
    Main game loop update:
      - Bullets never limited; misses counted only outside cheat mode
      - Health & power-up pickups spawn & update
      - Enemies respawn immediately on player collision (and on bullet kill)
      - Difficulty scales by +3 spawns/wave & +10% speed/wave (cap wave 10)
      - Shields & upgrades timers tick down
    """
    ...
    # — Update bullets & count misses (only if not cheat) —
    new_bullets = []
    for b in bullets:
        b.update()
        if b.is_alive():
            ...
            if not b.hit_enemy and not cheat_mode:
                bullets_missed += 1
    ...
    # — Spawn power-ups (shield / upgrade) —
    if random.random() < 0.01 and len(power_ups) < 2:
        if random.random() < 0.5:
            color, ptype = (0.6, 0.2, 0.8), 'shield'
        else:
            color, ptype = (0.0, 0.9, 0.9), 'upgrade'
        pu = Pickup(
            random.uniform(-BOUNDARY_SIZE/2+50, BOUNDARY_SIZE/2-50),
            random.uniform(-BOUNDARY_SIZE/2+50, BOUNDARY_SIZE/2-50),
            color, ptype
    ...
    # — Ensure enough enemies alive —
    max_enemies = MAX_ENEMIES_BASE + (current_wave - 1) * 3
    while len(enemies) < max_enemies:
        enemies.append(Enemy())

    # — Enemy movement, collision & respawn on hit/player-contact —
    new_enemies = []
    for e in enemies:
        e.update()

        # --- Player collision: damage + immediate respawn ---
        if e.check_collision_with_player():
            ...
            continue  # skip bullet logic for this frame

        # --- Bullet collision: kill + immediate respawn ---
        hit = False
        for b in bullets:
            if not b.hit and e.check_collision_with_bullet(b):
                ...
                game_score += 1
                zombies_killed += 1
                show_notification("Zombie killed!", (1,1,1))
                e.respawn()
                hit = True
                break
        if not hit:
    ...
    # — Wave progression every 5 kills, up to wave 10 —
