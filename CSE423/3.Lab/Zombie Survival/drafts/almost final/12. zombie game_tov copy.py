...
def update_game():
    ...
    Main game loop update:
      - Infinite bullets; misses counted only outside cheat mode
      - Health & power‐up pickups spawn & update
      - Enemies immediately respawn on bullet hit or player collision
      - Blood splatter effect on bullet hit
      - Difficulty scales: +3 zombies per wave, +10% speed per wave (cap 10)
      - Shield & weapon timers tick down
    ...
    # — Bullet logic (misses only when not in cheat mode) —
    new_bullets = []
    for b in bullets:
        b.update()
        if b.is_alive():
            ...
            if not b.hit_enemy and not b.cheat_bullet:
                bullets_missed += 1
    ...
    # — Ensure enough enemies for this wave —
    max_enemies = MAX_ENEMIES_BASE + (current_wave - 1) * 3
    while len(enemies) < max_enemies:
        enemies.append(Enemy())

    # — Enemy movement & collision with immediate respawn —
    new_enemies = []
    for e in enemies:
        e.update()

        # Player collision ⇒ damage + respawn
        ...
        # Bullet collision ⇒ blood splatter + respawn
        hit = False
        for b in bullets:
    ...
    # — Update all particles (blood, etc.) —
    update_particles()
