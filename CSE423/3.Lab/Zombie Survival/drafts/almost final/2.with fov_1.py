...
def draw_player():
    """
    In 3rd-person mode, draw the full rig (legs, torso, head, arms + 3rd-person gun).
    In 1st-person (follow_camera=True), skip the body entirely.
    """
    global follow_camera, player_pos, player_angle, game_over, player_height

    if follow_camera:
        # FPS: we don’t draw the full player body
        return

    glPushMatrix()
    # position & rotate to match world coords
...
def draw_gun():
    """
    In 1st-person (follow_camera=True), reset to camera space and draw just
    arms+gun via draw_first_person_gun_hand_head().
    In 3rd-person, draw a simple offset cylinder.
    """
    global follow_camera, player_pos, player_height, player_angle

    if follow_camera:
        # first-person overlay: position at camera and draw arms+gun
    ...
    # third-person gun bolted to the back of the torso
...
def draw_first_person_gun_hand_head(cx, cy, cz):
    """
    First-person view: draw both fore-arms and a long barrel pointing out of the screen.
    """
    glMatrixMode(GL_MODELVIEW)
    glPushMatrix()
    glLoadIdentity()
    # move into the camera’s coordinate space
    ...
    glTranslatef(6, -8, -12)
    glRotatef(90, 1, 0, 0)
    gluCylinder(quad, 2.5, 2.5, 20, 12, 2)
    glPopMatrix()
    ...
    # main barrel
    gluCylinder(quad, 2.0, 2.0, 40, 16, 2)
    # muzzle extension
    glPushMatrix()
    glTranslatef(0, 0, -40)
    gluCylinder(quad, 1.2, 1.2, 20, 12, 1)
    glPopMatrix()
    glPopMatrix()
