...
def draw_barrel_fp():
    # switch to MODELVIEW and save
    glMatrixMode(GL_MODELVIEW)
    glPushMatrix()
    # wipe out the world‑space camera transform so this draws in camera space
    glLoadIdentity()

    # ─── now your original transforms ───
    glRotatef(player_angle, 0, 1, 0)       # 1) rotate to player yaw
    glTranslatef(0, 15, -10)               # 2) drop it down a bit
    glTranslatef(0, 0, -18)                # 3) push it forward
    glRotatef(-10, 1, 0, 0)                # 4) tilt slightly downward
    ...
    # restore the previous MODELVIEW (with camera)
    glPopMatrix()
...
def draw_player():
    ...
    # your existing third‐person gun
    draw_gun()
...
def draw_gun():
    """
    First-person: a simple barrel that follows your position & yaw.
    Third-person: your old TPV gun.
    """
    glMatrixMode(GL_MODELVIEW)
    glPushMatrix()

    if follow_camera:
        # 1) move to the player's eye position
        glTranslatef(
            player_pos[0],
            player_pos[1] + player_height,  # your eye-height
            player_pos[2]
        )

        # 2) rotate to match the player's view direction
        glRotatef(player_angle, 0, 1, 0)

        # 3) shove the barrel out in front of you:
        #    tweak these three values until it sits just under your crosshair
        glTranslatef(
            0.0,    # left/right offset   (0=centered)
           -1.0,    # vertical offset     (negative = down)
           -2.0     # forward offset      (negative = into scene)
        )

        # 4) draw a cylinder along the camera's forward (-Z) axis
        glPushMatrix()
        # by default gluCylinder points +Z → we want -Z, so flip 180° around Y:
        glRotatef(180, 0, 1, 0)
        ...
        gluCylinder(quad,
                    0.1,   # base radius
                    0.1,   # top radius
                    1.5,   # length of barrel
                    12, 3) # slices, stacks
        glPopMatrix()

    else:
        # --- your old third-person gun: ---
...
def setupCamera():
    ...
    if follow_camera:
        ...
        eye_x = player_pos[0]
        eye_y = player_pos[1]  + 35   # ← raised from “+ player_height + 10”
        eye_z = player_pos[2]
...
def showScreen():
    ...
    # if FP mode, draw only the barrel in camera‐space
    if follow_camera and not game_over:
        draw_barrel_fp()
    else:
        # in TP or when game_over: draw the full player model
        draw_player()
