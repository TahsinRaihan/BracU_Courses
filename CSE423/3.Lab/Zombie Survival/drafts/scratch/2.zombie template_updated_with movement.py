...
class Enemy:
    ...
    def draw(self):
        ...
        glPopMatrix()
        # Head (with eyes)
        glColor3f(0.05, 0.05, 0.05) if not self.is_dead else glColor3f(0.2, 0.1, 0.1)
        glPushMatrix()
        glTranslatef(0, 42, 0)
        ...
        glPopMatrix()
        # Arms
        glColor3f(0.4, 0.3, 0.2) if not self.is_dead else glColor3f(0.2, 0.1, 0.1)
        for x_arm in [-15, 15]:
            glPushMatrix()
...
class HealthPickup:
    ...
    def draw(self):
        # Draw pinkish-red heart shape as health orb
        glPushMatrix()
...
class PowerUp:
    ...
    def draw(self):
        if not self.active:
            return
        # Draw cyan triangle as weapon orb
        glPushMatrix()
...
def inside_boundary(x,z):
    ...
    return -half<= half and -half<= half
...
def draw_player():
    ...
    # Gun in hand - simple cylinder
    glColor3f(0.2, 0.2, 0.2)
    glPushMatrix()
    # gun position: right hand approx
    glTranslatef(12, 52, 8)
    glRotatef(-player_angle, 0, 1, 0)
    glRotatef(-90, 1, 0, 0)
    quad = gluNewQuadric()
...
def mouseListener(button, state, x, y):
    global follow_camera, fov, fov_adjust_active
...
def mouseMotion(x, y):
    ...
    if fov_adjust_active:
        # Adjust FOV according to vertical mouse position
        height = glutGet(GLUT_WINDOW_HEIGHT)
        ratio = max(min(y / height, 1), 0)
        fov = 25 + 100 * (1 - ratio)  # FOV between 25 and 125
...
def init():
    glClearColor(0.02, 0.02, 0.15, 1.0)  # Dark night sky blue
